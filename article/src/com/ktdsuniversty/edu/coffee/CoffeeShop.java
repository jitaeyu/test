package com.ktdsuniversty.edu.coffee;

public class CoffeeShop {
	
	Coffee hot;
	Coffee ice;

	public CoffeeShop(Coffee hot, Coffee ice) {
		this.hot=hot;
		this.ice=ice;
	}
	
	/**
	 * @param menu 메뉴들의 번호. 1:hot , 2:ice
	 * @param quantity 주문수량
	 * @return 주문가격
	 * */
	
	public int orderCoffee(int menu, int quantity) {
		if(menu==0) {
			int getCoffeeStock = hot.getStrock();
			if(getCoffeeStock>=quantity) {
				hot.setStock((getCoffeeStock-quantity));
				return hot.getPrice()*quantity;
			}else {
				int getIceteaStock = hot.getStrock();
				if(getIceteaStock>=quantity) {
					hot.setStock((getIceteaStock-quantity));
					return hot.getPrice()*quantity;
				}
			}
//			System.out.println(this.hot.name+"음료를"+quantity+"개 주문받았습니다");
//			return this.hot.price * quantity;
		}else if (menu==1) {
			int getCoffeeStock = ice.getStrock();
			if(getCoffeeStock>=quantity) {
				ice.setStock((getCoffeeStock-quantity));
				return ice.getPrice()*quantity;
			}else {
				int getIceteaStock = ice.getStrock();
				if(getIceteaStock>=quantity) {
					ice.setStock((getIceteaStock-quantity));
					return ice.getPrice()*quantity;
				}
			}
//			System.out.println(this.ice.name+"음료를"+quantity+"개 주문받았습니다");
//			return this.ice.price * quantity;
		}else {
			System.out.println("존재하지 않는 음료입니다");
			return 0;
		}
		return 0;
	}
	
}
